...
function Index(){
	...
	if($this->db->table_exists('whatsapp_notificacoes')){
		$filtro_tipo_equipe = $this->db->field_exists('tipo_notificacao', 'whatsapp_notificacoes')
			? " WHERE tipo_notificacao NOT IN ('equipe_confirmado', 'equipe_cancelado', 'equipe_remarcado')"
			: "";
...
function buscar_paciente() {
	...
	$sql = "
		SELECT u.id, u.nome, u.telefone,
		       ...
		       (SELECT a2.data_agenda FROM agendamentos a2 WHERE a2.id_paciente = u.id ORDER BY a2.data_agenda DESC, a2.id DESC LIMIT 1) AS ultima_data,
		       (SELECT a2.tipo       FROM agendamentos a2 WHERE a2.id_paciente = u.id ORDER BY a2.data_agenda DESC, a2.id DESC LIMIT 1) AS ultimo_tipo,
		       (SELECT a2.data_agenda FROM agendamentos a2 WHERE a2.id_paciente = u.id AND a2.data_agenda >= '{$hoje}' AND a2.status != 3 ORDER BY a2.data_agenda ASC, a2.hora_agenda ASC, a2.id ASC LIMIT 1) AS proximo_data,
		       (SELECT a2.tipo       FROM agendamentos a2 WHERE a2.id_paciente = u.id AND a2.data_agenda >= '{$hoje}' AND a2.status != 3 ORDER BY a2.data_agenda ASC, a2.hora_agenda ASC, a2.id ASC LIMIT 1) AS proximo_tipo
		FROM usuarios u
		WHERE u.nivel = 5
	...
	foreach ($this->db->query($sql)->result() as $row) {
		$result[] = [
			...
			'ultimo_tipo'  => htmlspecialchars((string)$row->ultimo_tipo, ENT_QUOTES, 'UTF-8'),
