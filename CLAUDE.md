...
- **Status de deploy (2026-09-01):** código em `origin/main` (commit `74d00b1`). Pendente em produção: (1) subir por FTP os 7 arquivos runtime (`application/config/{routes,whatsapp}.php`, `application/controllers/Cron.php`, `application/controllers/adm/Dev.php`, `application/helpers/whatsapp_agendamento_helper.php`, `application/libraries/Whatsapp_agendamento.php`, `application/models/Whatsapp_model.php`); (2) rodar `adm/dev/migrar_lembrete_whatsapp` (nível 1) — até lá o endpoint responde `ok` mas não envia (guarda em `field_exists`); (3) cadastrar o cron horário no cPanel (`wget -q -O /dev/null "https://utecnologia.com.br/cron/lembrete-whatsapp?token=notwa10230901marlusti"`). Chamada sem token válido responde 403 `forbidden` (comportamento esperado).
