...
- **Lembrete automático (cron):** `GET /cron/lembrete-whatsapp?token=...` (`Cron::lembrete_whatsapp()`), agendado de hora em hora no cPanel, dispara um lembrete único ao **paciente** quando faltam até 7h para a consulta (`status = 0`), pulando quem já recebeu o mesmo tipo, quem já confirmou e quem recebeu a confirmação há menos de 3h. A lane do profissional existe no código mas fica atrás do flag `whatsapp.lembrete_profissional_ativo` (env `WHATSAPP_LEMBRETE_PROFISSIONAL=1`) até o template dedicado sem botões ser aprovado. Token em `application/config/whatsapp.php` (`WHATSAPP_CRON_TOKEN`). Log em `whatsapp_notificacoes` com `tipo_notificacao` (`confirmacao` | `lembrete_paciente` | `lembrete_profissional`). MVP reusa o template de confirmação; templates dedicados: `docs/whatsapp-lembrete-templates-pendente.md`. Migração: `adm/dev/migrar_lembrete_whatsapp`.
