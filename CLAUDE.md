...
- **Status de deploy (2026-09-02):** em produção — 7 arquivos runtime enviados, `adm/dev/migrar_lembrete_whatsapp` executada e cron horário cadastrado no cPanel. Endpoint sem token válido responde 403 `forbidden` (esperado). Pendente: aprovar na Meta os 2 templates dedicados (`docs/whatsapp-lembrete-templates-pendente.md`) e, opcionalmente, mover o token para a env `WHATSAPP_CRON_TOKEN` no cPanel (hoje usa o fallback do arquivo).
