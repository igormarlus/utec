...
- **Status de deploy:**
  ...
  - **2026-09-22** — notificação WhatsApp à equipe: templates `agendamento_confirmado_equipe` / `agendamento_cancelado_equipe` aprovados na Meta; código implementado, pendente de envio por FTP e smoke test em produção. Publicar os 6 arquivos runtime tocados: `application/config/whatsapp.php`, `application/helpers/whatsapp_agendamento_helper.php`, `application/libraries/Whatsapp_agendamento.php`, `application/controllers/Webhooks.php`, `application/models/Whatsapp_model.php`, `application/controllers/adm/Atendimento.php` — os dois últimos excluem as linhas `equipe_confirmado`/`equipe_cancelado` das consultas de "última resposta"; sem eles a etiqueta "Confirmado/Cancelado via WhatsApp" da agenda fica vulnerável a ser mascarada pelas novas linhas. Nota: `WHATSAPP_NOTIFICAR_EQUIPE` já sai ligado por padrão — não é preciso setar nada no cPanel; `=1` é um no-op inofensivo, só `=0` muda o comportamento (não "corrigir" isso como se faltasse um passo).
